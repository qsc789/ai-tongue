 ...
 */
@RestController
@RequestMapping("face/pre")
public class DemoPreFaceController {
    private Logger log = LoggerFactory.getLogger(DemoPreFaceController.class);
    private final AiTongueProperties aiTongueProperties;
    private final IRestTemplateService restTemplateService;
    // 结果接收地址前缀，完整路径形如：https://your.domain/face/pre/resultReturn
    private final static String RETURN_URL_PREFIX = "https://your.domain";
    // 第三方单据id
    private Integer outId = 2;

    @Autowired
    public DemoPreFaceController(AiTongueProperties aiTongueProperties, IRestTemplateService restTemplateService) {
     ...
     */
    @PostMapping("taskPre")
    public String taskPre() throws Exception {
        // 1.获取accesstoken
        ...
        form.setReturnUrl(RETURN_URL_PREFIX + "/face/pre/resultReturn");
        File faceImg = new File(this.getClass().getClassLoader().getResource("static/face_main.jpg").getFile());
        File faceLeftImg = new File(this.getClass().getClassLoader().getResource("static/face_left.png").getFile());
        File faceRightImg = new File(this.getClass().getClassLoader().getResource("static/face_right.png").getFile());
        ...
        ResultVO resultVO = restTemplateService.postForObject(aiTongueProperties.getFacePreTaskUrl(), headers, params, ResultVO.class);
        log.info("face pre task:{}", JsonUtil.toJson(resultVO));
        return AiTongueConstant.OK;
    }


    @PostMapping("taskDo")
    public String taskDo() throws Exception {
        // 1.获取accesstoken
        String accesstoken = this.getAccessToken(aiTongueProperties.getDevId(), aiTongueProperties.getDevSecret());
        if (StringUtils.isBlank(accesstoken)) {
            return AiTongueConstant.ERROR;
        ...
        // （1）resultVO.data表示预计等待检测时间，
        // （2）检测的结果会回传到您设置的returnUrl上
        ResultVO resultVO = restTemplateService.postForObject(aiTongueProperties.getFaceDoTaskUrl(), headers, params, ResultVO.class);
        log.info("face do task:{}", JsonUtil.toJson(resultVO));
        return AiTongueConstant.OK;
    }
